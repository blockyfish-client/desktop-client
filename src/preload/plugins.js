...
function createPluginBox(
	...
	// Update button
	const ub = `<button id="${
		name.split(" ").join("_").toLowerCase() + "_update_button"
	}" class="plugin-action-button update">
...
<button id="${
		name.split(" ").join("_").toLowerCase() + "_delete_button"
	}" class="plugin-action-button delete">
	<svg style="pointer-events: none" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
	...
	// For online plugins
	const db = `<button id="${
		name.split(" ").join("_").toLowerCase() + "_download_button"
	}" class="plugin-action-button download">
	<svg style="pointer-events: none" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
...
function createPluginsModal() {
	...
	document
		.querySelectorAll(".plugin-modal-box .plugin-modal-close")
		.forEach((el) => {
	...
	document
		.querySelectorAll(".plugin-modal-box .plugin-folder-open")
		.forEach((el) => {
			el.addEventListener("click", () => {
	...
	window.allPlugins.forEach((plugin) => {
		try {
			...
			var ts = document.querySelector(
				`.plugin-list input#${
					plugin.name.split(" ").join("_").toLowerCase() + "_switch"
				}[type="checkbox"]`
			);
			if (
				getSettings(
					"plugins." +
						plugin.name.split(" ").join("_").toLowerCase() +
						".enabled"
				)
			) {
			...
			if (updatable) {
				...
				ub.addEventListener("click", async (e) => {
					...
					await downloadPlugin(
						`${config.remoteEndpoint}/plugins${plugin.url}`
					);
...
function downloadPlugin(url) {
	return new Promise(async (resolve) => {
		...
		fs.writeFileSync(
			path.join(app.getPath("userData"), "plugins", fn),
			plugin
		);
		resolve(plugin);
	});
