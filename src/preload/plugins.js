...
function createPluginsModal() {
	const plugin_modal = document.createElement("div");
	...
	for (const el of document.querySelectorAll(".plugin-modal-box .plugin-modal-close")) {
	...
	for (const el of document.querySelectorAll(".plugin-modal-box .plugin-folder-open")) {
		el.addEventListener("click", () => {
		...
		(async () => {
			...
			for (const plugin of window.plugins) {
				try {
					...
					if (getSettings(`plugins.${module.name.split(" ").join("_").toLowerCase()}.enabled`)) {
						module.script();
			...
			for (const plugin of window.allPlugins) {
				installedPlugins.push({
					id: plugin.id,
					version: plugin.versionNumber,
				});
			}
			for (const plugin of window.remotePlugins) {
				try {
					const installed = installedPlugins.find((p) => p.id === plugin.id);
					if (!installed) {
						window.onlinePlugins.add(plugin);
					} else if (installed.version < plugin.versionNumber) {
...
function renderPluginList() {
	...
	for (const plugin of window.allPlugins) {
		try {
			const updatable = window.updatablePlugins.has(plugin.id);
			document
				.querySelector(".plugin-list")
				.appendChild(createPluginBox(plugin.name, plugin.description, plugin.author, plugin.version, false, false, updatable));
			const ts = document.querySelector(`.plugin-list input#${plugin.name.split(" ").join("_").toLowerCase()}_switch[type="checkbox"]`);
			...
			const rb = document.querySelector(`.plugin-list button#${plugin.name.split(" ").join("_").toLowerCase()}_delete_button`);
			rb.addEventListener("click", async (e) => {
				modified = true;
				if (!plugin.fn) {
					try {
						let temp_fn = "";
						for (const e of window.remotePlugins) {
							if (e.name === plugin.name) {
								temp_fn = e.url.split("/").pop();
							}
						}
						if (temp_fn !== "") {
							plugin.fn = temp_fn;
			...
			if (updatable) {
				const ub = document.querySelector(`.plugin-list button#${plugin.name.split(" ").join("_").toLowerCase()}_update_button`);
	...
	for (const plugin of window.onlinePlugins) {
		try {
			document
				.querySelector(".plugin-list")
				.appendChild(createPluginBox(plugin.name, plugin.description, plugin.author, plugin.version, false, true, false));
			const db = document.querySelector(`.plugin-list button#${plugin.name.split(" ").join("_").toLowerCase()}_download_button`);
...
function getPlugins() {
	return new Promise((resolve) => {
		...
		let plugins = fs.readdirSync(path.join(app.getPath("userData"), "plugins"), {
			encoding: "utf8",
		});
