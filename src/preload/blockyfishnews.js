...
(async () => {
	...
	(async () => {
		let news = await (await fetch("https://blockyfish.vercel.app/blockyfishfeed/news.txt")).text();
		blockyfish_news.innerHTML = news;
	})();
	(async () => {
		let tut = await (await fetch("https://blockyfish.vercel.app/blockyfishfeed/tutorial.txt")).text();
